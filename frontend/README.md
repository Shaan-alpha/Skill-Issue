...
- **`npm run test:run`** — Vitest 4 + happy-dom 20 + Testing Library (added in v0.6.0; happy-dom bumped to v20 on 2026-05-22, Vitest 3→4 in the v1.0.x security slice). 73 unit + snapshot tests cover the OG palette, data fetchers, OgCard, CardActions, events wiring, PostHog provider, FramerProvider, the `<RefreshButton>` state machine (v0.8.2), the `/api/revalidate` route (v0.8.6), the Sentry PII scrub (v1.0.4), OG/card ingest attribution (v1.0.5), the `APP_VERSION`↔`package.json` drift guard (v1.0.7), and the SSE done-sentinel completion (v1.0.7).
