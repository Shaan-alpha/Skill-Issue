...
- **#62** — audit findings 2 and 3 (`badge-row`, same file so they ship together). **The most useful thing learned all session:** the reset the Next guide prescribes for transient open/closed state — `setOpen(false)` in a `useLayoutEffect` cleanup — **does not work for portaled content**. Implemented it first, it did nothing; instrumented it and found the cleanup *does* run, but the update is scheduled against a subtree React has just hidden, hidden Activity content re-renders at low priority, and `data-open` is still on the popup afterwards. For in-boundary DOM that is invisible because the boundary is `display: none` anyway; a portal has nothing hiding it in the meantime. Fix drops the positioner out of the layout **synchronously** in the cleanup, then closes the state for the re-show — which is what the same guide already does for its global-styles (`style.media`) and video (`video.pause()`) examples, for exactly this reason. Finding 3 was the simpler half: `nativeButton={false}`, since `Badge` renders a `<span>` and Base UI's default of `true` left the element with no role at all.
...
2. **`badge-row.tsx` — a pinned-open badge popover survives the route being hidden.** *Fixed in #62.* `Popover.Portal` mounts into `document.body`, *outside* the Activity boundary, so the boundary's `display: none` does not reach it. Probe confirmed the evidence text is still in `document.body` after the hide. Narrower than it first looks: Base UI closes on outside-press, so clicking a nav link closes it first — the reachable path is the **browser back button** (or keyboard nav) while a popover is pinned. Worth a real-browser confirm via the `verify` skill before fixing.
3. **`badge-row.tsx:15` — the badge trigger is not exposed as a button (a11y, unrelated to Activity).** *Fixed in #62.* The code comment claims *"Popover.Trigger renders a `<button>`"*; it no longer does. Probe: `tag: SPAN, role: null, exposed as button to a11y tree: false`, and Base UI itself warns `expected a native <button> because the nativeButton prop is true`. Focusable via `tabindex=0`, but screen readers announce no role. Almost certainly a Base UI upgrade changed this under a comment nobody re-read. Fix is likely `nativeButton={false}` or rendering a real `<button>`.
...
**Next:** merge **#60 first** (it is what makes CI green again), then #59, #61 and #62 in any order — they share no files. Then the operator decides whether this ships as v1.0.11 (bump 4 constants + PLAN row + CHANGELOG heading) or rides along with the next slice. All three audit findings are now fixed. All `CHANGELOG`/`PROGRESS_LOG` entries for all four PRs live in **#59** deliberately, so the branches do not conflict on these two files.

**Still worth doing, not done:** #62 is verified at the jsdom level only — the `verify` skill (headless-Chrome CDP) would confirm finding 2's fix against a real browser, where Base UI's outside-press close means the reachable path is narrow (browser-back or keyboard nav while a popover is pinned). And `backend/pyproject.toml` remains on **1.0.3** against 1.0.10 everywhere else; the v1.0.7 drift guard only asserts `package.json == APP_VERSION`, so extending it to the backend pair would stop that recurring.
