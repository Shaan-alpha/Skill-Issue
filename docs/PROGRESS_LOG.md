...
**Slice:** v1.0.8 (implemented, unreleased/untagged — paused for operator go-ahead)
...
**Learned / surprises:** Confirmed the `npm audit` restore is still blocked — latest Next.js is **16.2.12**, still inside the vulnerable advisory range (fix `>16.3.0-preview.7`, unreleased).
**Blocked / open:** PR → CI → prod-verify, then tag `v1.0.8` — **paused for operator go-ahead**. Standing: restore the `npm audit` gate once Next.js patches; remaining deferred items (SI-15/30, CSP-enforcing, SBOM) in PLAN.md.
**Next:** Open the PR. The audit remediation is now essentially complete — only the explicitly-deferred product/ops-gated items remain.
