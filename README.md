...
Pre-alpha. Latest shipped release is **v0.8.2** (Manual "Force refresh" button on `/me` — synchronous re-ingest via `POST /me/refresh/{username}`, 10/hour per-user rate limit, Layer A cache write-through). Live at https://skill-issue-tau.vercel.app — GitHub OAuth sign-in, Neon Postgres persistence, `/me` history, opt-in `/share/[slug]` public links. The AI narrative layer (Roast + Mentor) runs on **Groq** (`llama-3.3-70b-versatile`). v0.7.0 added Upstash Redis caching (warm `/analyze` ≤ 200 ms); v0.7.2 prod-certified the perf budget (CLS 0.080 → **0** structurally, perf 90 → 94, LCP 2,804 → 2,773 ms); v0.8.0 shipped Sentry (FE+BE), PostHog (events + web vitals), structlog JSON logging, on-voice 404, and a full axe a11y pass (0 critical / 0 serious / 0 moderate); v0.8.1 ships the nightly cron with bearer auth + Sentry capture on rate-limit cliff; v0.8.2 pairs it with the impatient-case button. **v0.8.3 — on-demand `revalidateTag` for `/share/[slug]` ISR** is next, then `vercel.ts` migration (v0.8.4). See [`CHANGELOG.md`](./CHANGELOG.md) for shipped slices, [`PLAN.md`](./PLAN.md) for the full roadmap, and [`docs/PROGRESS_LOG.md`](./docs/PROGRESS_LOG.md) for the most recent session handoff.
