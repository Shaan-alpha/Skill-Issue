...
Pre-alpha. Latest shipped release is **v0.8.4** (hotfix — narrative-persistence honesty: `is_fallback` and `provider` now derived correctly so analytics aren't lying; narrative-mode CHECK constraint trimmed to `('roast','mentor')`; GitHub `User-Agent` tracks the live version instead of `0.1.0`). Live at https://skill-issue-tau.vercel.app — GitHub OAuth sign-in, Neon Postgres persistence, `/me` history, opt-in `/share/[slug]` public links. The AI narrative layer (Roast + Mentor) runs on **Groq** (`llama-3.3-70b-versatile`). v0.7.0 added Upstash Redis caching (warm `/analyze` ≤ 200 ms); v0.7.2 prod-certified the perf budget (CLS 0.080 → **0** structurally, perf 90 → 94, LCP 2,804 → 2,773 ms); v0.8.0 shipped Sentry (FE+BE), PostHog (events + web vitals), structlog JSON logging, on-voice 404, and a full axe a11y pass; v0.8.1 ships the nightly cron with bearer auth; v0.8.2 pairs it with the manual force-refresh button on `/me`; v0.8.3 hotfixes the empty-repo crash caught by Sentry; v0.8.4 fixes the silent narrative misattribution. **v0.8.5 — CI pipeline + `requirements.txt` cleanup** is next, then `revalidateTag` ISR (v0.8.6) and the `vercel.ts` migration (v0.8.7). See [`CHANGELOG.md`](./CHANGELOG.md) for shipped slices, [`PLAN.md`](./PLAN.md) for the full roadmap, and [`docs/PROGRESS_LOG.md`](./docs/PROGRESS_LOG.md) for the most recent session handoff.
