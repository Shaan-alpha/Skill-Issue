...
- **The narrator now spends its allowance on writing instead of thinking.** Raising the allowance treated the symptom; the model was still spending two-thirds of it thinking before writing a word. Measured against the live model: 386 tokens of thinking to 186 of visible text. Turning the thinking effort down to its lowest setting cuts that to 12 tokens and leaves the rest for prose — so a roast can no longer be squeezed out by its own preamble. Adjustable via `NARRATIVE_REASONING_EFFORT`, and ignored by models that don't think, so the OpenAI path is unaffected.
- **A roast that comes back blank now says so and offers a retry.** If the narrator returned nothing at all, the card rendered as an empty box with no message, no error, and no way forward. Worse, that empty result was written to the cache and served back for the next 24 hours, so a single blip left one profile permanently blank. Empty results are no longer cached, are no longer treated as a cache hit when an older one is found, and now fall back to the stand-in text.
...
- **Badge evidence popups are now announced as buttons by screen readers.** They were reaching assistive technology as unlabelled text, so the evidence behind each badge was effectively unreachable without a mouse or touchscreen.
- **Removed a browser-side error that React warns can cause an infinite render loop.** It came from how the signed-in state was read on first paint, and had been treated as harmless log noise.
...
Internal CI hardening and a dependency refresh. No user-facing change.
...
- **Dependencies brought up to date** — 25 backend packages and 10 frontend packages, plus the CI action versions. Nothing changes in how the app behaves; this clears a backlog of pending security advisories in libraries the project depends on.

### Security
- Cleared 9 outstanding dependency advisories. The ones that remain are either not shipped to users at all (development-only tooling) or are waiting on an upstream release that does not exist yet — both categories are documented in `docs/DEPLOY.md` rather than left unexplained.
